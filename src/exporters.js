...
export function buildDocxBytes(resume){
  ...
  const paper=String(resume?.settings?.paper||'a4').toLowerCase()==='letter'?{w:12240,h:15840}:{w:11906,h:16838};
  const documentXml=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body>${body}<w:sectPr><w:pgSz w:w="${paper.w}" w:h="${paper.h}"/><w:pgMar w:top="1000" w:right="1000" w:bottom="1000" w:left="1000"/></w:sectPr></w:body></w:document>`;
