...
const sectionWordMap=resume=>{
  ...
  m.summary=txt(resume.summary);m.experience=(resume.experience||[]).reduce((n,e)=>n+txt(`${e.title} ${e.company} ${e.location} ${e.startDate} ${e.endDate}`)+bullet(e.bullets),0);m.education=(resume.education||[]).reduce((n,e)=>n+txt(`${e.degree} ${e.institution} ${e.startDate} ${e.endDate} ${e.details}`),0);m.skills=(resume.skillGroups||[]).reduce((n,g)=>n+txt(g.name)+(g.skills||[]).reduce((a,x)=>a+txt(x),0),0);m.projects=(resume.projects||[]).reduce((n,p)=>n+txt(`${p.name} ${p.role} ${p.startDate} ${p.endDate} ${p.url} ${p.description}`)+bullet(p.bullets),0);m.certifications=(resume.certifications||[]).reduce((n,c)=>n+txt(`${c.name} ${c.issuer} ${c.date} ${c.url}`),0);m.languages=(resume.languages||[]).reduce((n,l)=>n+txt(`${l.language} ${l.level}`),0);m.achievements=(resume.achievements||[]).reduce((n,a)=>n+txt(`${a.title} ${a.date} ${a.description}`),0);for(const [id,items] of Object.entries(resume.genericSections||{}))m[id]=(items||[]).reduce((n,it)=>n+txt(`${it.title} ${it.subtitle} ${it.location} ${it.startDate} ${it.endDate} ${it.url} ${it.description}`)+bullet(it.bullets),0);for(const s of resume.customSections||[])m[`custom:${s.id}`]=(s.items||[]).reduce((n,it)=>n+txt(`${it.title} ${it.subtitle} ${it.location} ${it.startDate} ${it.endDate} ${it.url} ${it.description}`)+bullet(it.bullets),0);return m;
};
